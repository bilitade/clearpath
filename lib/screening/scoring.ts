...
export function getCareLevelRationale(score: ScoreResult): string[] {
  const lines: string[] = [];

  lines.push(
    `PHQ-9 score of ${score.phq9Total} (${score.phq9Band.replace("_", " ")}) and GAD-7 score of ${score.gad7Total} (${score.gad7Band}).`,
  );

  if (score.isUrgent) {
    lines.push(
      "Your responses suggest higher distress — expedited care is recommended.",
    );
  } else if (score.careLevel === "psychiatry") {
    lines.push(
      "Severity suggests evaluation with a psychiatrist or specialized clinician.",
    );
  } else if (score.careLevel === "therapy") {
    lines.push(
      "Moderate symptoms often benefit from outpatient therapy with a licensed clinician.",
    );
  } else if (score.careLevel === "coaching") {
    lines.push(
      "Mild symptoms may respond well to coaching, brief support, or structured self-help.",
    );
  } else {
    lines.push(
      "Minimal symptoms may be managed with self-guided resources and monitoring.",
    );
  }
